...
def batchNorm(input: np.ndarray):
    """Batch normalization
    ...
    for c in input[0]:
        for w in input[2]:
            for h in input[1]:
                h = (h-input_mean)/input_std
    return input
    
def basicBlock(input:np.ndarray, step_size:int=1, padding_size:int = 1, kernel_size:int=3):
    
    normalized_input = batchNorm(input)
